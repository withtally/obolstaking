...
contract DelegateCompensationStakerTest is Test, PercentAssertions {
  ...
  {
    _boundedEarningPower = bound(_earningPower, 1, 1e27);
  }
...
contract InitializeDelegateCompensation is DelegateCompensationStakerTest {
  function testFuzz_InitializeSingleDelegateCompensationWithZeroEarningPower(address _delegate)
    public
  {
    _assumeValidDelegate(_delegate);
    calculator.setDelegateEarningPower(_delegate, 0);

    vm.expectEmit();
    emit DelegateCompensationStaker.DelegateCompensationInitialized(
      _delegate, Staker.DepositIdentifier.wrap(0), 0
    );
  ...
  function testFuzz_InitializeSingleDelegateCompensationWithNonZeroEarningPower(
  ...
  function testFuzz_InitializeMultipleDelegateCompensationWithZeroEarningPower(
    address _delegate1,
    address _delegate2
  ) public {
    ...
    calculator.setDelegateEarningPower(_delegate1, 0);
    calculator.setDelegateEarningPower(_delegate2, 0);

    vm.expectEmit();
    emit DelegateCompensationStaker.DelegateCompensationInitialized(
      _delegate1, Staker.DepositIdentifier.wrap(0), 0
    );
    Staker.DepositIdentifier depositId1 = staker.initializeDelegateCompensation(_delegate1);

    vm.expectEmit();
    emit DelegateCompensationStaker.DelegateCompensationInitialized(
      _delegate2, Staker.DepositIdentifier.wrap(1), 0
    );
  ...
  function testFuzz_InitializeMultipleDelegateCompensationWithNonZeroEarningPower(
    ...
    vm.expectEmit();
    emit DelegateCompensationStaker.DelegateCompensationInitialized(
      _delegate1, Staker.DepositIdentifier.wrap(0), _earningPower1
    );
    Staker.DepositIdentifier depositId1 = staker.initializeDelegateCompensation(_delegate1);

    vm.expectEmit();
    emit DelegateCompensationStaker.DelegateCompensationInitialized(
      _delegate2, Staker.DepositIdentifier.wrap(1), _earningPower2
    );
    Staker.DepositIdentifier depositId2 = staker.initializeDelegateCompensation(_delegate2);
  ...
  function testFuzz_RevertIf_DelegateCompensationIsInitializedTwice(
    uint256 _earningPower,
    address _delegate
  ) public {
    _assumeValidDelegate(_delegate);
    ...
    vm.expectRevert(
      abi.encodeWithSelector(
        DelegateCompensationStaker.DelegateCompensationAlreadyInitialized.selector, _delegate
      )
    );
    staker.initializeDelegateCompensation(_delegate);
  }
...
contract DisabledMethodsTest is DelegateCompensationStakerTest {
